...
router.get("/github/:username", (req: Request, res: Response) => {
  try{
    ...
    const options = {
      uri: `https://api.github.com/users/${username}/repos?per_page=5&sort=created:asc&client_id=${config.githubClientId}&client_secret=${config.githubSecret}`,
      method: "GET",
...
router.get("/me", auth, async (req: Request, res: Response) => {
  try{
    ...
    const profile = await Profile.findOne({user : user}).populate("user",["name","avatar"]);
    res.json(profile);
  }catch(error){
    console.error(error.message);
    res.status(500).send("Server Error");
