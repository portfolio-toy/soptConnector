...
router.post("/", auth_1.default, [
    ...
    try {
        let profile = await Profile_1.default.findOne({ user: user.id });
        if (profile) {
            // Update
            profile = await Profile_1.default.findOneAndUpdate({ user: user.id }, { $set: { value: profileFields } }, { new: true });
...
router.put("/experience", auth_1.default, [
    ...
    try {
        const profile = await Profile_1.default.findOne({ user: req.body.user.id });
